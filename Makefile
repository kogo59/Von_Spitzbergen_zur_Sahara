BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = Von_Spitzbergen_zur_Sahara
TITLE_NAME = "Von Spitzbergen zur Sahara"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA_PDF) $(CHAPTERS_HTML_PDF) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Von_Spitzbergen_zur_Sahara_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Von_Spitzbergen_zur_Sahara_*.jpg 
